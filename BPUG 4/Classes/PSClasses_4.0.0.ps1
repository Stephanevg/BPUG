...
Class Server : Computer {
    ...
    [void] ProvisionMachine(){
        if (!($this.Name)){
            throw "A machine name needs to be set prior to be able to provision it."
        }else{
            invoke-command -ComputerName woop -scriptBLock {Function:new-fastvm -CreateVM -VMName $this.Name -VMSize Medium -JoinDomain -OS Win2012R2}
        }
    }
}
...
#--> Working With Clients
    ...
    #Create in AD
        ...
        $BPUGServer.create('OU=Servers,OU=HQ')

    #Provision Server
        
        $BPUGServer.ProvisionMachine() #Will fail :(
